...
def get_current_location():
    ...
    g = geocoder.ip('me')  # Fetch the geolocation using the device's public IP
    if g.latlng:
        return g.latlng  # Returns [latitude, longitude]
    else:
        return None  # Fallback if geolocation fails

def send_help_request():
    # Fetch current location
    location = get_current_location()

    # Construct help message with location
    help_message = ""
    if location:
        help_message += f" Location: Latitude: {location[0]}, Longitude: {location[1]}"
    else:
        help_message += " Location: Unknown"

    # Write the help request to the file
    with open("help_request.txt", "w") as file:
...
GROQ_API_KEY = "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"
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("HELP!"):
            send_help_request()  # Call the function when button is pressed
            st.success("Help request sent to First Responder!")  # Show success message


        # Add spacing before the HELP! button
        
        
        # "HELP!" Button
        #help_button = st.markdown('<button class="help-button">HELP!</button>', unsafe_allow_html=True)
        #if help_button:
            #st.write("")  # Customize this action as needed
